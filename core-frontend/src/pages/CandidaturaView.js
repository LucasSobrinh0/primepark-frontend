...
function CandidaturaView() {
  ...
  const fetchCandidatos = async () => {
    try {
      let url = `http://127.0.0.1:8000/api/candidatura/list/?page=${page}`;
  ...
  const fetchApplications = async () => {
    try {
      const resp = await fetch('http://127.0.0.1:8000/api/vagas/application/');
      if (resp.ok) {
        const data = await resp.json();
  ...
  const fetchVagas = async () => {
    try {
      const resp = await fetch('http://127.0.0.1:8000/api/vagas/vaga/');
      if (resp.ok) {
        const data = await resp.json();
    ...
    if (appData.applicationId) {
      try {
        const resp = await fetch(
          `http://127.0.0.1:8000/api/vagas/application/update/${appData.applicationId}/`,
          {
            method: 'PATCH',
      ...
      try {
        const resp = await fetch(
          "http://127.0.0.1:8000/api/vagas/application/create/",
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
  ...
  const handleSaveObservacoes = async () => {
    ...
    try {
      ...
      const resp = await fetch(
        `http://127.0.0.1:8000/api/vagas/application/update/${appData.id}/`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
